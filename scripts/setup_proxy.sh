...
generate_proxy_list() {
    awk -F "/" '{print $3 ":" $4 ":" $1 ":" $2 }' "$WORKDATA" > /home/proxy-installer/proxy.txt
}

# Function to generate proxy data entries with default credentials
create_proxy_data() {
    local user_index=1
    seq "$FIRST_PORT" "$LAST_PORT" | while read -r port; do
        echo "usr${user_index}/default_password/$IP4/$port/$(generate_64bit_ip_address $IP6)"
